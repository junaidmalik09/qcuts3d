...
length_final_neighbours = 0;
return;
SE = strel('sphere', 1);



neighbourx1=suppixel; neighbourx2=suppixel; neighboury1=suppixel; neighboury2=suppixel; neighbourz1=suppixel; neighbourz2=suppixel;
...
neighbourz1(:,:,2:end)=suppixel(:,:,1:end-1); NZ1stats=regionprops(suppixel,neighbourz1,'PixelValues');
neighbourz2(:,:,1:end-1)=suppixel(:,:,2:end); NZ2stats=regionprops(suppixel,neighbourz2,'PixelValues');



final_neighbours=cell(1,max_label);
for label_counter=1:max_label
%     mask = suppixel==label_counter;
%     dilatedImage = imdilate(mask, SE);
%     borderPixels = bwperim(dilatedImage);
%     pixelValues = suppixel(borderPixels);
%     final_neighbours{label_counter}=unique(pixelValues);
    neighbours=zeros(1,max_label);
    neighbours([NX1stats(label_counter).PixelValues; NX2stats(label_counter).PixelValues; ])=1;
    neighbours([NY1stats(label_counter).PixelValues; NY2stats(label_counter).PixelValues; ])=1;
    neighbours([NZ1stats(label_counter).PixelValues; NZ2stats(label_counter).PixelValues; ])=1;
...
neighbourhood = final_neighbours;
return;
